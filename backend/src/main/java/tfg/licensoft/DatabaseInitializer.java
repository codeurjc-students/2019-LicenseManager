...
public class DatabaseInitializer {
	...
	public void init() throws StripeException {
		...
		User u1= new User("customer@email.com",c1.getId(),"customer", "c","ROLE_USER");
		userServ.save(u1);
	

		Customer c2 = Customer.retrieve("cus_GCfRldTX850sXN");
		User u2= new User("user@email.com",c2.getId(),"user", "u","ROLE_USER");
		userServ.save(u2);
		

		Customer c3 = Customer.retrieve("cus_GCfRu2cBr7Xkkh");
		User u3= new User("c1@email.com",c3.getId(),"C1", "c1", "ROLE_USER");
		userServ.save(u3);

		

		Customer c4 = Customer.retrieve("cus_GCfRVJhDGwsjpi");
		User u4= new User("admin@email.com",c4.getId(),"admin", "a",  "ROLE_USER", "ROLE_ADMIN");
		userServ.save(u4);	
